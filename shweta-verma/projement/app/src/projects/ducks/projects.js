...
export const fetchProjectsBypage = (pagevalue) => async dispatch => {
    dispatch(setIsLoading(true));
    let response;
    try {
        response = await (!pagevalue ? fetch(`/api/projects/`) : fetch(pagevalue)).then(res => res.json());
    }
    catch (e) {
        return console.error(e);
    }
    response.detail ? (dispatch(setIsLoading(false)), dispatch(receiveProjects(null))) : dispatch(receiveProjects(response));
